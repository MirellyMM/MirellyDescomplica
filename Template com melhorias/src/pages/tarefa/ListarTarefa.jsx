...
// Importando os componentes CriarTarefa e EditarTarefa
...
const ListarTarefa = () => {
  ...
  const handleOpenEditar = (id) => {
    const tarefaSelecionada = tarefas.find(t => t.idTarefa === id);
    if (tarefaSelecionada) {
      setIdTarefaSelecionada(id);
      setTarefa(tarefaSelecionada);
      setOpenEditar(true);
    } else {
      // Lógica para lidar com a tarefa não encontrada
    }
  };
  const handleCloseEditar = () => setOpenEditar(false);

  const handleEditar = (id) => {
    const index = tarefas.findIndex(t => t.idTarefa === id);
    if (index !== -1) {
      const novasTarefas = [...tarefas];
      novasTarefas[index] = { ...tarefas[index], ...tarefa };
  ...
  return (
    <>
      ...
      >
        <div>
          <CriarTarefa handleClose={handleClose} tarefas={tarefas} setT
